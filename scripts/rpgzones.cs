...
function RecursiveZone(%delay)
{
	...
	//echo("Zone check time:" SPC $LastZoneCheckDuration);
	schedule(%delay, 0, "RecursiveZone", %delay);
...
{
	%start = getRealTime();
	//echo("Time 1:" SPC %start);
	//echo("Zone check!");
	//InitContainerRadiusSearch ( "0 0 0" , 9999, $TypeMasks::PlayerObjectType );
	//%player = containerFindFirst( $TypeMasks::PlayerObjectType, "0 0 0", 9999,9999,9999);
	//echo(%player);
	//%player = containerSearchNext();
	//echo(%player);
	
	%playerList = nameToId("MissionGroup/Players");
	if(%playerList != -1)
	{
		%pcnt = %playerList.getCount();
		if(%pcnt > 0)
		{
			for(%i = 0; %i < %pcnt; %i++)
			{
	...
	//{
	//	%player.ZoneUpdate();
		//echo("Found player:" SPC %player);
	//	%player = containerSearchNext();
	//}
	%end = getRealTime(); 
	//echo("Time 2:" SPC %end);
	$LastZoneCheckDuration = %end - %start;
}

function Player::ZoneUpdate(%this)
{
	%client = %this.client;
	if( !isObject(%client))
		return;

	//Updating of zones is handled by triggers currently.
	
	//Weather effects go here.
	
	//Bonus state decrease goes here.
	//echo(%client);
	%pos = %this.position;
	//echo(%pos);
	...
	if(%pos != %client.zoneLastPos && %this.getState() !$= "Dead")
	{
		...
		if(OddsAre(8))
			UseSkill(%client, $SkillWeightCapacity, True, True, "", True);
		//%invis = !fetchData(%client, "invisible");
		//echo("Invis check:" SPC %invis);
		if(!fetchData(%client, "invisible")) //!fetchData(%client, "invisible"))
		{
			//echo("Not invisible");
			//%odds = OddsAre(mfloor(%client.data.PlayerSkill[$SkillSenseHeading] / 100)+1);
			//echo("Odds:" SPC %odds);
